...
import { buildProviderQuotaSnapshot, fetchGimso2xProxyUsageReports, type ProviderQuotaInventoryRow, type ProviderUsageReportLike } from './gjc-provider-quota.js';
import {
...
export class GjcBunSdkAdapter implements GjcWorkerRuntime {
  ...
  async #providerQuota(): Promise<JsonObject> {
    ...
    const quota: ProviderQuotaSnapshot = await buildProviderQuotaSnapshot({
      ...
      fetchProviderUsage: async (provider) => {
        if (provider === 'gimso2xproxy') {
          const custom = await fetchGimso2xProxyUsageReports(provider);
          if (custom !== undefined) return custom;
        }
        return await this.authStorage.fetchUsageReports({
          provider,
